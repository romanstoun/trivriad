...
canvas.addEventListener('touchstart', (e) => {
    ...
    console.log('Touch start:', row, col); // Для отладки
});
...
canvas.addEventListener('touchend', (e) => {
    ...
    if (selected) {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY)) {
            ...
            if (deltaX > 100 && c < cols - 1) newPos = [r, c + 1]; // Вправо (увеличен порог до 100)
            else if (deltaX < -100 && c > 0) newPos = [r, c - 1]; // Влево (увеличен порог до 100)
        } else {
            // Вертикальный свайп (вверх/вниз)
            if (deltaY > 100 && r > 0) newPos = [r - 1, c]; // Вверх (увеличен порог до 100)
            else if (deltaY < -100 && r < rows - 1) newPos = [r + 1, c]; // Вниз (увеличен порог до 100)
        }
    ...
    console.log('Touch end, delta:', deltaX, deltaY); // Для отладки
});
